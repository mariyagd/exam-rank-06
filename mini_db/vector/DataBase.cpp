...
DataBase::DataBase(const std::string & filePath) : _data(), _filePath(filePath), _response() {}
...
DataBase::~DataBase()
{
    ...
    if (!_file.is_open())
    {
        std::cerr << "open: " << std::string(strerror(errno));
        exit(1);
    }
...
void    DataBase::treatRequest(const int & fd, const std::string & buffer)
{
